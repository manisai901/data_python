...
EST = pytz.timezone(
    "Canada/Eastern")  # check your timezones https://gist.github.com/heyalexej/8bf688fd67d7199be4a1682b3eec7568
...
def _get_recently_played_tracks():
    ...
    con_str = 'mysql://' + str(connection.login) + ':' + str(connection.password) + '@' + str(
        connection.host) + ':' + str(connection.port) + '/' + str(connection.schema)
    engine = create_engine(
        con_str)  # Creating engine with the con_str (We are pulling all the variables with the help of the MySqlHook
...
with DAG(
        ...
        schedule='10 4 * * *',
        # This DAG will run at 10 min past mindnight (12:10)EST, The rason I have build 4:10 because my airflow follows UTC timestamp
):
